...
namespace Services.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task SendVerifyCode(SendVerifyCodeDto sendVerifyCodeDto)
        {
            ...
            switch (sendVerifyCodeDto.SelectedProvider)
            {
        ...
        public async Task<UserViewModel> ChangePasswordAsync(string userId,ChangePasswordDto model)
        {
            //get user
            var user = await _userService.GetUserAsync(userId);
            //change Password
            ...
            {
                List<IdentityError> errorList = changePassResult.Errors.ToList();
                var errors = string.Join(", ", errorList.Select(e => e.Description));
                throw new AppException($"{errors}");
            }
            //map to editUser model for update
